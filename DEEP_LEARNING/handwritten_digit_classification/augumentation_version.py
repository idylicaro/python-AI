...
generator_training = ImageDataGenerator(rotation_range=7, horizontal_flip=True, shear_range=0.2, height_shift_range=0.07, zoom_range=0.2)
generator_test = ImageDataGenerator()

base_training = generator_training.flow(predictors_training, targets_training, batch_size=128)
base_test = generator_test.flow(predictors_test, targets_test, batch_size=128)

# steps_per_epoch=(total of images / batch_size)
classifier.fit_generator(base_training, steps_per_epoch= 600000 / 128, epochs=5, validation_data=base_test, validation_steps=10000 / 128)
